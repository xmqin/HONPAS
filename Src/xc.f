...
C      Shanghui and xmqin , HSE06 part following cp2k code.
C         'HSE06' = >  hybrid  Latest HSE06 version: omega = 0.11 Bohr^-1
C                              Krukau et al, JCP 125, 224106 (2006)
C         'PBE0'  = >  hybrid  Ernzerhof & Scuseria, JCP 110, 5029 (1999);
C                              Adamo & Barone, JCP 110, 6158 (1999).
...
C derivatives with respect to density and density gradient
          if (GGAfunc) then
            if (XCauth(nf).eq.'HSE06' .or. XCauth(nf).eq.'hse06'.or.
     .          XCauth(nf).eq.'PBE0' .or. XCauth(nf).eq.'pbe0' ) then
C-----------PBE0 and HSE06 use PBE pseudopotential -------------!!!!
               CALL GGAXC( 'PBE', IREL, nspin, D, GD,
...
cjdg
     ...
     .               EPSX, EPSC, DEXDN, DECDN, DEXDGN, DECDGN )
      ELSE IF (AUTHOR.EQ.'HSE06' .OR. AUTHOR.EQ.'hse06') THEN
        CALL HSEXC( IREL, NS, DD, GDD,
     .              EPSX, EPSC, DEXDN, DECDN, DEXDGN, DECDGN )
      ELSE IF (AUTHOR.EQ.'PBE0' .OR. AUTHOR.EQ.'pbe0') THEN
...
C Set output arguments
   ...
   90 CONTINUE
      ...
      SUBROUTINE HSEXC( IREL, nspin, Dens, GDens,
     .                  EX, EC, DEXDD, DECDD, DEXDGD, DECDGD )

C *********************************************************************
C Implements HSE Generalized-Gradient-Approximation.
C Ref: JOURNAL OF CHEMICAL PHYSICS VOLUME 120, NUMBER 16 (2004)
C edited from  L.C.Balbas and J.M.Soler. December 1996. Version 0.5.
c Coded by shanghui, 2010.04.01
C Modified by xmqin, 2013,09.12
C Set omega as an input parameter read from .fdf  
C ******** INPUT ******************************************************
C INTEGER IREL           : Relativistic-exchange switch (0=No, 1=Yes)
C INTEGER nspin          : Number of spin polarizations (1 or 2)
C REAL*8  Dens(nspin)    : Total electron density (if nspin=1) or
...
C Internal variables
     ...
     .  T, THD, THRHLF, TWO, TWOTHD, VCUNIF(2), VXUNIF(2), ZETA
      ...
      real(dp) D_left,KFS_left,S_left,FSR_left,
     .         D_right,KFS_right,S_right,FSR_right 
