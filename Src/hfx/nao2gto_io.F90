...
&                   (real(hfx_opts%npts_fit, dp) - 1)
          ...
          if( l > 0)  hfx_opts%max_num_gaus = 3
          write(6,*) inlz,  hfx_opts%max_num_gaus
...
!          gtos(isp)%pgf_radius(ipgf,inlz)= exp_radius(l, zeta, 1.d-10, gcca)
          gtos(isp)%pgf_radius(ipgf,inlz)= exp_radius(l, zeta, 1.d-4, 1.0_dp ) ! gcca) !1.0_dp)
!!         For debugging
          write(6,'(a,3i5,f12.5)')                                           &
...
&       "The fitting procedure was not successfully completed"
  ...
  subroutine nao2gto_print_info(gtos, hfx_opts)
    ...
    ! Local variables
    integer :: inlz, isp, jnlz, nf, lun2, n_series, zeta, ispol, npts, ip, irad
    real(dp) :: dummy
...
&                 (real(hfx_opts%npts_fit, dp) - 1)
 ...
 &            trim(spp%label), spp%orbnl_n(inlz), spp%orbnl_l(inlz),    &
              spp%orbnl_z(inlz), gtos(isp)%orbnl_contract(inlz)
          write(lun2,'("#",5X,A)') "exponent_gaussian1 coefficient_gaussian1"
          write(lun2,'("#",5X,A)') "exponent_gaussian2 coefficient_gaussian2"
          write(lun2,'("#",5X,A)') "exponent_gaussian3 coefficient_gaussian3"
          do jnlz=1,gtos(isp)%orbnl_contract(inlz)
            write(lun2,'("#",E18.8,1X,E18.8)')gtos(isp)%orbnl_zeta(jnlz,inlz), &
              gtos(isp)%orbnl_coefficient(jnlz,inlz)
          enddo
          write(lun2,'("#",5X,A)')     &
 &           "r, linear combination Gaussians/r^l, orb/r^l"
          fval(:) = 0.0_dp
          do jnlz = 1, gtos(isp)%orbnl_contract(inlz)
 ...
 &                    exp(-rad_pts(:)**2*gtos(isp)%orbnl_zeta(jnlz,inlz))
          ...
          do ip = 1, npts
            write(lun2,'(3g26.16)')rad_pts(ip), fval(ip), orb_pts(ip)
          enddo 
         call io_close(lun2)
      enddo 
    enddo 
