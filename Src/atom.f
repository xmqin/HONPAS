...
CCC ----- added by xmqin for HSE06,  Nov. 2013------------------------------------
          elseif((XCauth(nf).eq.'hse06').or.(XCauth(nf).eq.'HSE06')
     .                 .and.(XCfunc(nf).eq.'GGA'))
     .        then

            write(6,'(a)')
     .       'xc_check: HSE06: JCP 125, 224106 (2006)'
            if (icorr.ne.'ps'.and.nXCfunc.eq.1)
     $          write(6,'(a,1x,2a)')
     .          'xc_check: WARNING: Pseudopotential generated with',
     $           trim(ps_string), " functional"
CCC-----------------------------------------------------------------
          elseif((XCauth(nf).eq.'pbe0').or.(XCauth(nf).eq.'PBE0')
     .                 .and.(XCfunc(nf).eq.'GGA'))
     .        then

            write(6,'(a)')
     .       'xc_check: PBE0: JCP 110, 5029 (1999); 110, 6158 (1999)'
            if (icorr.ne.'ps'.and.nXCfunc.eq.1)
     $          write(6,'(a,1x,2a)')
     .          'xc_check: WARNING: Pseudopotential generated with',
     $           trim(ps_string), " functional"
