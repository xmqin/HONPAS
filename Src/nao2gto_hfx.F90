...
!      neris_tmp       = 0_i8b
      ...
      if(hfx_optdata%Dynamic_parallel) then
      buf  =  11
      request = 1000
      tag = 111
      task_frag = hfx_optdata%frag_size
...
!                    spher_eri_calc = spher_eri_calc + 1
                    if ( DM_max*dabs(eri(nsoi,nsoj,nsok,nsol)*2.0_dp) .ge. &
...
!                     time_tot =time_tot + time2 -time1
        ...
        endif ! index_ij > index_kl
      enddo  ! i_list_ijkl
    enddo    ! do while

    endif !

   else ! parallel setting (dynamic or static)

!    if(node .eq.0) write(6,*) "Static parallel NAO2GTO"
#ifdef MPI
...
!    if(node.eq.0)
!     write(6,*) "HFX time : ", time2-time1, " s  on ", node
!!   For debugging
!    write(6,'(a,i20)') &                             
