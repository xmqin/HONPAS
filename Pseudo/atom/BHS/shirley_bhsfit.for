c           s,p,d, f correspond to 1 3 5 7
c           p_so, d_so, f_so correspond to 2 4 6
c [ Everything in Hartree ]
c
c  AG (based on ELS description)
c------------------------------------------------------------
c
	dimension vi(nrmax,7),vcore(nrmax),r(nrmax)
        dimension c(6),aa(6),q(6,6),s(6,6),a(3)
        write (6,*) 'PLEASE ENTER ZV,A1,A2,C1,C2 FOR CORE POTENTIAL.'
	read (5,*) zv,a1,a2,c1,c2
 	ra1=dsqrt(a1)
 	ra2=dsqrt(a2)
 	do 25 i=1,nr
 	vcore (i)=-zv/r(i)*(c1*derf(ra1*r(i))+c2*derf(ra2*r(i)))
 25	continue
 30	write (6,*) 'YOU MAY NOW ENTER WHAT YOU WANT TO ENTER.'
 ...
 110	  continue
	...
	1             -(aa(2)+rr*aa(5))*dexp(-a(2)*rr)
	1             -(aa(3)+rr*aa(6))*dexp(-a(3)*rr)
 235	    continue
	  else
	    do 240 i=1,nr
	    rr=r(i)*r(i)
